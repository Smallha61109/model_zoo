...
def predict_seq(seq_images, checkpoint, labels, diff):
  ...
  #  poses.append(labels[1])
  #  poses.append(labels[2])
  #  poses.append(labels[3])
  #  poses.append(labels[4])
  #  poses.append(labels[5])
  #  poses.append(labels[6])
  #  poses.append(labels[7])
  #  poses.append(labels[8])
  #  poses.append(labels[9])
  #  poses.append(labels[10])
  if os.path.isfile(checkpoint + '.index') and \
    ...
    with tf.Session() as sess:
      ...
      for i in range(len(seq_images) - diff):
        ...
        acc = abs(labels0-prediction[0])/labels0+\
              abs(labels1-prediction[1])/labels1+\
              abs(labels2-prediction[2])/labels2
        acc_all = acc_all + acc
